...
# from computeclient.v1 import client as compute_client
...
def apiclient(request):
    ...
    try:
        api_url = base.url_for(request, 'instances')
    except exceptions.ServiceCatalogException:
        LOG.debug('No instances Management service is configured.')
        return None

    LOG.debug('computeclient connection created using the token "%s" and url'
              '"%s"' % (request.user.token.id, api_url))
    # enable following after client product implemented.
    # c = compute_client.Client(
...
def instances_create(request, **kwargs):
    args = {}
    ...
    # created = apiclient(request).instancess.create(**args)
...
def instances_update(request, id, **kwargs):
    args = {}
    for (key, value) in kwargs.items():
        if key in ATTRIBUTES:
            args[str(key)] = value
            ...
            raise exceptions.BadRequest(
                "Key must be in %s" % ",".join(ATTRIBUTES))
    # updated = apiclient(request).instances.update(id, **args)
...
def instances_delete(request, id):
    # deleted = apiclient(request).instancess.delete(id)
...
def instances_list(
    ...
    # list = apiclient(request).instancess.list(limit, marker, sort_key,
    #                                             sort_dir, detail)
    list = [STUB_DATA[data] for data in STUB_DATA]
    return list


def instances_show(request, id):
    # show = apiclient(request).instancess.get(id)
