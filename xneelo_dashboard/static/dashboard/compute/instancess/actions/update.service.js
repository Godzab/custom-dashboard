...
(function() {
  ...
  /**
   * @ngdoc overview
   * @name horizon.dashboard.compute.instancess.update.service
   * @description Service for the instances update modal
   */
  angular
    .module('horizon.dashboard.compute.instancess')
    .factory('horizon.dashboard.compute.instancess.update.service', updateService);
  ...
  updateService.$inject = [
    ...
    'horizon.dashboard.compute.instancess.model',
    'horizon.dashboard.compute.instancess.resourceType',
    'horizon.dashboard.compute.instancess.workflow'
  ];
  ...
  function updateService(
    ...
    var message = {
      success: gettext('instances %s was successfully updated.')
    };
    ...
    function perform(selected, newScope) {
      ...
      title = gettext("Update instances");
      ...
      // load current data
      id = selected.id;
      var deferred = api.getinstances(id);
    ...
    function allowed() {
      ...
      //return policy.ifAllowed({ rules: [['instances', 'update_instances']] });
    }

    function submit() {
      model.cleanProperties();
      return api.updateinstances(id, model.spec).then(success);
    }
